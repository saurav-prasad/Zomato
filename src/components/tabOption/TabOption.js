                    ...
                    {tabs.map((tab) => {
                        return (
                            ...
                            <div onClick={() => setactiveTab(tab.name)}
                                className={`cursorPointer flexCenter tabItem ${activeTab === tab.name && "activeTab"}`}
                            >
                                <div className='tabImageContainer flexCenter'
                                    style={{ backgroundColor: `${activeTab === tab.name ? tab.backdrop:'#F8F8F8'}` }}>
                                    <img src={activeTab === tab.name ? tab.active_img : tab.inactive_img} className='tabImg' alt={tab.name} />
                                </div>
                                <p className='tabName userSelect' style={{color: `${activeTab === tab.name ? '#EF4F5F':"#696969"}`}}>{tab.name}</p>
                            </div>
                        )
                    })}
                </div>
